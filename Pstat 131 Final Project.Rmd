...
# Data Cleaning
In this step we add a row where 1 denotes a player getting drafted and 0 denotes a player not getting drafted.
```{r}
stats <- college_stats %>% mutate(drafted = ifelse(is.na(pick), 0, 1))
```

Here we clean the data so it only includes a players last season as well as rename some of the predictors. 
```{r}
...
Cleaning abnormal values in data set and changing them to NA.
```{r}
...
Height is a unique format using months to denote feet. For example May corresponds to 5 feet, June to 6 feet, and July to 7 feet. This column also contains a number that denotes inches. The month and number for inches are separated by a dash and in this step we clean this columns so that height is listed in just inches.
```{r}
#changing ht
...
This histogram shows the average points scored per game on the x-axis and the frequency number of players in the data set that score average that number of points. Presumably players with more points per game are more likely to be drafted and this histogram shows how common or uncommon a players scoring ability is.
```{r}
...
To better understand the relationship between our predictors, we make scatterplots with 2 predictors on the axis and color indicating whether that player was drafted.
...
In the plot below we see a similar effect with rim shot percentage and rim shots made. Where rim shots made appears to be more informative for the same reasons because of lack of data for certain players.
```{r}
...
It looks like the lowest oob error comes from the model where mtry=2
...
rf.err
print(paste('Random Forrest Error: ', test.rf.err))
```
...
test.log.err = 1 - sum(diag(log.err))/sum(log.err)
log.err
print(paste('Logistic Regression Error: ', test.log.err))
```

```{r}
boosted_predict <- predict(boosted_fit, test_set, type = "class")

table_boosted <- table(pred = boosted_predict, true = test_set$drafted)
table_boosted

test.boost.err <- round((1-(sum(diag(table_boosted)) / sum(table_boosted))), 8)
print(paste('Boosted Trees Error: ', test.boost.err))
...
test.error = mean(predYtst != YTest)
knn.err <- table(pred=predYtst, true=test_set$drafted)
knn.err
print(paste('K-Nearest-Neighbor Error: ', test.error))
```

# Conclusion

The model with the lowest error when making predictions for the trst set is 